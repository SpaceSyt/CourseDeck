"""User-initiated, bounded headless course browsing. No arbitrary code or selectors."""
...
class BrowserTools:
    def __init__(self, service, task_tools, expose):
    ...
    def definitions(self, define):
        ...
        return [
            ...
            define(
                "browser_open",
                "Open a task or source course in the dedicated headless browser. "
                "Use IDs from get_task/browser_courses. Supply exactly one ID. "
                "Returns visible evidence and allowed navigation targets; never submits work.",
                {"task_id": string, "source_course_id": string},
                [],
            ),
            define(
                "browser_follow",
    ...
    async def run(self, name, args):
        schemas = {
            "browser_courses": set(),
            "browser_open": {"task_id", "source_course_id"},
    ...
    async def open(self, args):
        if len(args) != 1 or not all(isinstance(v, str) and v for v in args.values()):
            raise ValueError("Choose exactly one task_id or source_course_id")
        task = self.tasks.get(args["task_id"]) if "task_id" in args else None
        cid = task.get("source_course_id") if task else args["source_course_id"]
        ...
        url = task.get("url") if task else course.get("source_url")
    ...
    async def follow(self, args):
        ...
        if target["kind"] == "document":
            ...
            document = await self.service.link_reader.read(
                ...
                {"link_id": args["target_id"], "url": target["url"], "title": target["label"]},
            )
            ...
            try:
                ...
                for candidate in controls.values():
                    item = {
                        ...
                        for key in ("label", "kind", "role", "href")
                    }
    ...
    def is_document(self, url):
        ...
        if self.course["provider"] == "brightspace":
            return bool(
                static_file_path(clean, self.connector.base_url, self.course["external_id"])
            )
        return False
