...
完成首次安装后，Windows 双击 **start-tray.vbs**。它会在系统托盘显示 CourseDeck 图标，
并打开浏览器中的界面，不显示命令行窗口。关闭浏览器后，托盘仍在时会继续后台同步。

托盘菜单：

- **Open CourseDeck**：打开界面。
- **Sync now**：立即同步。
- **Exit**：退出托盘，停止由托盘启动的本地服务。

重复启动托盘入口会打开已有界面。如果服务已通过命令行运行，请先关闭命令行服务，再启动托盘。
Settings → Preferences → **Start at Windows sign-in** 可以设置当前用户登录后的自动启动，默认关闭。
自动启动只显示托盘，不打开网页；关闭开关即可取消。电脑休眠或尚未登录时不会同步。
移动项目目录前先关闭自启动，移动后再打开。
...
然后打开 **http://127.0.0.1:48321**。关闭运行窗口或按 Ctrl+C 即停止。
...
托盘也支持端口和数据目录参数，例如
`.\.venv\Scripts\pythonw.exe -m coursedeck.tray --port 48322 --data-dir "D:\CourseDeck data"`。
默认数据目录固定为项目根目录下的 `data`，不随启动时的工作目录改变。
可用 `--data-dir` 或 `COURSEDECK_DATA_DIR` 指定其他目录；命令行参数优先。
同一数据目录只允许一个服务进程，即使使用不同端口；端口和目录检查在启动同步之前完成。
改变数据目录不会自动迁移数据和系统凭据。关闭服务或电脑休眠期间不会同步，
后台活动也不能延长平台规定的强制登录期限。
...
后台同步会先尝试复用专用浏览器的登录会话，等待学校 SSO 返回，并处理已识别的 Google 账号选择、
Google / Microsoft 已填邮箱的 Next。多个账号无法确定时不代选；Gmail 只选择已绑定账号。
这些步骤不依赖 AI，也不自动填写密码、验证码或同意新增授权。真正需要验证时仍需手动完成，
缓存会保留；登录中转未加载完则按网络异常重试，不直接视为登录失效。
...
- 课程平台每轮完成后间隔 10 分钟自动同步；服务启动后首次定时同步等待 10 分钟。
